...
describe('Technicians route: ', () => {
  ...
  it('should get all technicians', async () => {
    const res = await request(server).get('/technicians');
  ...
  it('should create a new technician and return the same technician when all are fetched', async () => {
    const post_res = await request(server).post('/technician').set('Content-Type', 'application/json').send(mocks.mockIncompleteUser4);
    ...
    const get_res = await request(server).get('/technicians');
  ...
  it('should create a new technician, return the same technician when all are fetched and delete the technician successfully', async () => {
    const post_res = await request(server).post('/technician').set('Content-Type', 'application/json').send(mocks.mockUser2);
    expect(post_res.statusCode).equal(201);
    ...
    const del_res = await request(server).delete(`/technician/${id}`);
